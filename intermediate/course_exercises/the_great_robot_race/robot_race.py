...
while not rr.is_race_over(bots) and num_of_turns < max_turns:
  ...
  for bot in bots:
    if bot.has_finished == False:
      bot_decision = rr.compute_robot_logic(walls, goal, bot)
      robot_moves.append(bot_decision)
  num_of_turns += 1
...
while len(robot_moves) > 0:
  ...
  # Add your code below!
  bot_name, bot_movement, bot_collided = robot_moves.popleft()
  bot_data[bot_name].process_move(bot_movement)
